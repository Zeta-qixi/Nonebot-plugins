...
import re
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), 'data','spider'))
...
async def switch(session: CommandSession):
    if session.ctx['user_id'] in session.bot.config.MASTER:
        ...
        plugins = comman[1]
        global new_switch,hbook_switch,hpic_switch
        if swtich=='开启':
            if plugins == '新闻':
                new_switch = True
            elif plugins == '本子':
...
@on_command('jtz', aliases=('日语翻译','j2z'), only_to_me=False)                 #changed
...
@on_command('etz', aliases=('英语翻译','e2z'), only_to_me=False)                 #changed
async def ja_to_zh(session: CommandSession):
    if ' ' in session.ctx['raw_message']: 
        msg=session.ctx['raw_message'][5:]
...
@on_command('ztj', aliases=('翻译日语','z2j'), only_to_me=False)                 #changed
async def ja_to_zh(session: CommandSession):
    if ' ' in session.ctx['raw_message']: 
        msg=session.ctx['raw_message'][5:]
...
@on_command('zte', aliases=('翻译英语','z2e'), only_to_me=False)                 #changed
async def ja_to_zh(session: CommandSession):
    if ' ' in session.ctx['raw_message']: 
        msg=session.ctx['raw_message'][5:]
...
async def hbooks(session: CommandSession):
    if hbook_switch: 
        ...
        if '没有搜索到相关的内容' in responce:
            n_msg='没有找到关键词[{}]相关的本子哦'.format(f_msg)
